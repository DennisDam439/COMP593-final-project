...
class APODApp:
    ...
    def listloadimages(self):
        try:
            ...
            if titles:
                ...
                self.root.after(0, self.select_first_image)  # Select the first item
    ...
    def select_first_image(self):
        if self.image_combobox['values']:
            first_title = self.image_combobox['values'][0]
            self.image_combobox.set(first_title)
            threading.Thread(target=self.loadimages_display, args=(first_title,)).start()
    ...
    def loadimages_display(self, title):
        try:
            apod_info = apod_desktop.get_by_title(title)
            if apod_info:
